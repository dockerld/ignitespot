...
import {
  ...
  SLACK_SUPPORT_TICKET_MENTION_USER_IDS,
...
export function registerSlackHandlers(app) {
  ...
  app.view("support_ticket_submit", async ({ ack, body, view, client }) => {
    ...
    const requesterId = body.user?.id || "";
    let requesterName = body.user?.name || body.user?.username || "Unknown";
    if (requesterId) {
      try {
        const userInfo = await client.users.info({ user: requesterId });
        requesterName =
          userInfo.user?.real_name || userInfo.user?.profile?.real_name || requesterName;
      } catch (err) {
        console.error("⚠️ Could not fetch user info for requester:", err.message);
      }
    }
    const mentionValues = [];
    if (requesterId) {
    ...
    if (SLACK_SUPPORT_TICKET_MENTION_USER_IDS.length) {
      for (const mentionId of SLACK_SUPPORT_TICKET_MENTION_USER_IDS) {
        mentionValues.push(`<@${mentionId}>`);
      }
    } else if (SLACK_DEFAULT_MENTION_USER_ID) {
      mentionValues.push(`<@${SLACK_DEFAULT_MENTION_USER_ID}>`);
    } else {
      mentionValues.push("@docker");
    }
    ...
    // DM each support ticket mention user with a link to the Google Sheet
    if (message?.ts && GOOGLE_SHEETS_SPREADSHEET_ID) {
      const sheetUrl = `https://docs.google.com/spreadsheets/d/${GOOGLE_SHEETS_SPREADSHEET_ID}/edit`;
      const dmText = `📋 New support request from *${companyName}*\n*Type:* ${typeLabel} | *Priority:* ${priorityLabel}\n\n<${sheetUrl}|Manage support requests here>`;
      const usersToNotify = SLACK_SUPPORT_TICKET_MENTION_USER_IDS.length
        ? SLACK_SUPPORT_TICKET_MENTION_USER_IDS
        : SLACK_DEFAULT_MENTION_USER_ID
          ? [SLACK_DEFAULT_MENTION_USER_ID]
          : [];
  ...
  app.view("client_loss_submit", async ({ ack, body, view, client }) => {
    ...
    // Notion dual-write: update Close Out in Notion too
    try {
      const { findNotionRecordByField, updateNotionRecordFields, NOTION_CLIENT_SOFTWARE_DB_ID } = await import("../integrations/notion.js");
      if (NOTION_CLIENT_SOFTWARE_DB_ID && airtableCompany && airtableCompany !== "Not set") {
        const notionRecord = await findNotionRecordByField("Group Name", airtableCompany, NOTION_CLIENT_SOFTWARE_DB_ID);
        if (notionRecord?.id) {
          await updateNotionRecordFields(notionRecord.id, { "Close Out": "Disengaging" }, NOTION_CLIENT_SOFTWARE_DB_ID);
          console.log("📘 Notion close out updated:", notionRecord.id);
        }
      }
    } catch (notionErr) {
      console.error("⚠️ Notion close out dual-write error (non-fatal):", notionErr?.message || notionErr);
    }
  ...
  app.options("airtable_record_search", async ({ ack, payload }) => {
    ...
    try {
      // Phase 2: Search Notion first, fall back to Airtable
      const { searchNotionRecords, formatNotionOption, NOTION_CLIENT_SOFTWARE_DB_ID: notionSwDbId } = await import("../integrations/notion.js");
      let options = [];

      if (notionSwDbId) {
        const notionResults = await searchNotionRecords(query, notionSwDbId);
        options = notionResults.map((record) => formatNotionOption(record));
        console.log("📘 Notion search results:", options.length);
      }

      // Fall back to Airtable if Notion returned nothing
      if (!options.length) {
        const airtableOverrides = {
          searchFields: ["Sub Name (from Contact)"],
          displayField: "Sub Name (from Contact)",
          secondaryField: "",
        };
        ...
        console.log("📗 Airtable fallback results:", options.length);
      }
  ...
  app.action("qbo_accept_invite", async ({ ack }) => {
    await ack();
  });

  app.action("wrong_access_email", async ({ ack, body, client }) => {
    await ack();
    ...
    let clientData = {};
    try {
      clientData = JSON.parse(buttonValue);
    } catch (e) {
      console.error("❌ Could not parse wrong_access_email button value:", e.message);
    }

    const { companyName, senderName, senderEmail, inviteType } = clientData;

    if (channel && messageTs) {
      if (senderEmail) {
        try {
          const { sendEmail } = await import("../integrations/gmail.js");
          const type = (inviteType || "qbo").toLowerCase();
          const displayName = senderName || companyName || "there";
          const fromEmail = "qbo@ignitespot.com";

          let subject, emailBody;
          if (type === "xero") {
            subject = "Xero Invite – Incorrect Access Level";
            emailBody = `Hi ${displayName},\n\nThank you for sending over the Xero invite. It looks like the access level may not have come through correctly, which is required for our team to properly support you.\n\nTo resend the invite with the correct access, please follow the steps below:\n\n1. Log into Xero\n2. Click on the organisation name, select Settings, then click Users.\n3. Click Invite a user.\n4. Enter the new user's first name, last name, and email address.\n   First Name: Ignite\n   Last Name: Spot\n   Email: xero@ignitespot.com\n5. Select which features you want them to access, and a user role for each feature.\n   Please use the "Advisor" role and enable Bank Account Admin and Manage Users features\n6. (Optional) Click Add a message, then enter an email message.\n7. Click Send invite.\n\nOnce completed, please let us know so we can confirm receipt on our end.\n\nThank you, and we appreciate your help in getting this updated so we can continue moving forward with your onboarding.`;
          } else {
            subject = "QuickBooks Online Invite – Incorrect Access Level";
            emailBody = `Hi ${displayName},\n\nThank you for sending over the QuickBooks Online invite. It looks like the access level may not have come through as an Accountant (Accounting Firm) invite, which is required for our team to properly support you.\n\nTo resend the invite with the correct access, please follow the steps below:\n\n1. From your QuickBooks platform, select the gear/settings wheel in the top right corner.\n2. Select "Manage Users."\n3. Click on "Accounting Firms" (next to "Users").\n4. Press "Invite."\n5. Add Ignite Spot using the following information:\n   Email: QBO@ignitespot.com\n   First Name: Ignite\n   Last Name: Spot\n6. Select "Finish," and you're all set!\n\nOnce completed, please let us know so we can confirm receipt on our end.\n\nThank you, and we appreciate your help in getting this updated so we can continue moving forward with your onboarding.`;
          }
          ...
          await client.chat.postMessage({
            channel,
            thread_ts: messageTs,
            text: `<@${user}> flagged this as the wrong access email.\n✅ Correction email sent to *${senderEmail}* from *${fromEmail}* with ${type === "xero" ? "Xero" : "QBO"} instructions.`,
          });
        } catch (err) {
          console.error("❌ Wrong access email send error:", err?.message || err);
          await client.chat.postMessage({
            channel,
            thread_ts: messageTs,
            text: `<@${user}> flagged this as the wrong access email.\n❌ Failed to send correction email: ${err?.message || "unknown error"}`,
          });
        }
      } else {
        await client.chat.postMessage({
          channel,
          thread_ts: messageTs,
          text: `<@${user}> flagged this as the wrong access email.\n⚠️ No client email address found — please send the instructions manually.`,
        });
      }
    }
  });
