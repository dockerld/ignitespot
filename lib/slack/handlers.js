...
export function registerSlackHandlers(app) {
  ...
  app.view("support_ticket_submit", async ({ ack, body, view, client }) => {
    ...
    const selectedCompany =
      values.support_company?.support_company_search?.selected_option?.text?.text ||
      "";
    const companyName = manualCompany || selectedCompany;
    ...
    const errors = {};
    if (!companyName) {
      errors.support_company_manual = "Select a company or type one.";
    }
    if (Object.keys(errors).length) {
      await ack({ response_action: "errors", errors });
      return;
  ...
  app.options("support_company_search", async ({ ack, payload }) => {
    const raw = payload.value || "";
    const query = raw.trim().replace(/\s+/g, " ");
    console.log("🔎 support_company_search:", query);
    ...
    try {
      ...
      console.error(
        "❌ Airtable support company search error:",
        err?.response?.status,
        err?.response?.data || err
      );
      await ack({ options: [] });
