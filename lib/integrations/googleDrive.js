...
export async function ensureCompanyFolder(companyName) {
  ...
  if (existing) {
    ...
    for (const pathParts of CLIENT_FOLDER_TEMPLATE_PATHS) {
      await ensureFolderPath(existing.id, pathParts, token, childrenCache);
    }
  ...
  const childrenCache = new Map();
  for (const pathParts of CLIENT_FOLDER_TEMPLATE_PATHS) {
    await ensureFolderPath(created.id, pathParts, token, childrenCache);
  }
