...
import {
  ...
  HUBSPOT_DEAL_SLACK_THREAD_PROPERTY,
...
const dealPipelineCache = new Map();
...
function normalizeAlphaNum(value) {
  return normalizeHubSpotLabel(value).replace(/[^a-z0-9]+/g, "");
}

function parseHubSpotBoolean(value) {
  if (typeof value === "boolean") return value;
  if (value === null || value === undefined) return null;
  const normalized = normalizeHubSpotLabel(String(value));
...
export async function searchHubSpotDealTypes(rawQuery) {
  ...
  const HIDDEN_DEAL_TYPES = ["afc client"];
  const options = (await getHubSpotDealTypeOptions()).filter((option) => {
    const label = (option.label || "").toLowerCase();
    return !HIDDEN_DEAL_TYPES.includes(label);
  });
...
function pickMostRecentOpenDeal(deals) {
  const open = (deals || []).filter((deal) => {
    const closed = parseHubSpotBoolean(deal?.properties?.hs_is_closed);
    return closed === false;
  });
  return pickMostRecentDeal(open);
}

async function getHubSpotDealPipeline(pipelineId) {
  if (!HUBSPOT_TOKEN) {
    throw new Error("HubSpot token missing.");
  }
  ...
  if (dealPipelineCache.has(id)) {
    return dealPipelineCache.get(id);
  }

  const resp = await axios.get(`https://api.hubapi.com/crm/v3/pipelines/deals/${id}`, {
    headers: {
      Authorization: `Bearer ${HUBSPOT_TOKEN}`,
      "Content-Type": "application/json",
    },
  ...
  const pipeline = resp.data || null;
  dealPipelineCache.set(id, pipeline);
  return pipeline;
}

function pickClosedWonStageId(pipeline) {
  const stages = pipeline?.stages || [];
  if (!stages.length) return "";

  const target = "closedwon";
  const candidates = stages
    .map((stage) => {
      const labelNorm = normalizeAlphaNum(stage?.label);
      const idNorm = normalizeAlphaNum(stage?.id);
      const md = stage?.metadata || {};
      const isClosed = parseHubSpotBoolean(md?.isClosed) === true;
      const prob = Number(md?.probability);

      const labelLooksClosedWon =
        labelNorm === target ||
        labelNorm.includes(target) ||
        (labelNorm.includes("closed") && labelNorm.includes("won")) ||
        (labelNorm.includes("won") && isClosed);
      const idLooksClosedWon = idNorm === target || idNorm.includes(target);
      const looksClosedWon = labelLooksClosedWon || idLooksClosedWon;
      const looksProbClosedWon =
        isClosed && Number.isFinite(prob) && prob >= 1;

      let score = 0;
      if (looksClosedWon) score += 100;
      if (looksProbClosedWon) score += 25;
      if (isClosed) score += 10;
      if (labelNorm === target || idNorm === target) score += 10;

      return { stage, score };
    })
    .sort((a, b) => b.score - a.score);

  return candidates[0]?.stage?.id || "";
}

async function resolveClosedWonStageIdForPipeline(pipelineId) {
  const pipeline = await getHubSpotDealPipeline(pipelineId);
  return pickClosedWonStageId(pipeline);
}
...
export async function updateHubSpotDealStage(dealId, stageId) {
  ...
  if (!stageId) {
    return { updated: false, reason: "missing_stage_id" };
  }
  ...
  return { updated: true, dealId, stageId: String(stageId) };
}

export async function createClientLossDealForCompany(
  companyId,
  {
    dealTypeLabel = "Client Loss",
    pipelineId = HUBSPOT_SALES_PIPELINE_ID,
    associatePrimaryContact = true,
    nameSuffix = "",
  } = {}
) {
  if (!companyId) {
    return { created: false, reason: "missing_company_id" };
  }

  // Keep name unique-ish while still readable in HubSpot.
  const isoDate = new Date().toISOString().slice(0, 10);
  const company = await getHubSpotCompanyById(companyId, ["name"]);
  const companyName = company?.properties?.name || "Client Loss";
  const suffix = nameSuffix ? ` ${String(nameSuffix).trim()}` : "";
  const dealname = `${companyName} - Client Loss (${isoDate})${suffix}`.trim();

  let closedWonStageId = "";
  if (pipelineId) {
    try {
      closedWonStageId = await resolveClosedWonStageIdForPipeline(pipelineId);
    } catch (err) {
      console.error(
        "❌ HubSpot pipeline lookup error:",
        err?.response?.status,
        err?.response?.data || err
      );
    }
  ...
  const result = {
    ...
    pipelineId: pipelineId || "",
    stageId: closedWonStageId || "",
    dealTypeUpdate: { updated: false, reason: "not_attempted" },
    stageUpdate: { updated: false, reason: "not_attempted" },
    companyAssociation: { associated: false, reason: "not_attempted" },
    contactAssociation: { associated: false, reason: "not_attempted" },
  };

  if (!result.dealId) {
    return { created: false, reason: "missing_deal_id" };
  }

  try {
    await ensureHubSpotObjectsReady({ companyId, dealId: result.dealId });
  } catch (err) {
    // Non-fatal: associations/updates may still work without waiting.
    console.error(
      "❌ HubSpot object readiness wait error:",
      err?.response?.status,
      err?.response?.data || err
    );
  }
  ...
  try {
    result.companyAssociation = await associateDealToCompany(result.dealId, companyId);
  } catch (err) {
    console.error(
      "❌ HubSpot associate deal→company error:",
      err?.response?.status,
      err?.response?.data || err
    );
    result.companyAssociation = { associated: false, reason: "error" };
  }

  if (associatePrimaryContact) {
    ...
    if (primaryContactId) {
      try {
        result.contactAssociation = await associateDealToContact(
          result.dealId,
          primaryContactId
        );
      } catch (err) {
        console.error(
          "❌ HubSpot associate deal→contact error:",
          err?.response?.status,
          err?.response?.data || err
        );
        result.contactAssociation = { associated: false, reason: "error" };
      }
    } else {
      result.contactAssociation = { associated: false, reason: "missing_primary_contact" };
    }
  } else {
    result.contactAssociation = { associated: false, reason: "skipped" };
  }

  try {
    result.dealTypeUpdate = await updateHubSpotDealType(result.dealId, dealTypeLabel);
  } catch (err) {
    console.error(
    ...
    result.dealTypeUpdate = { updated: false, reason: "error" };
  }

  // If we couldn't resolve Closed Won stage ahead of time (e.g. missing pipeline env),
  // resolve the created deal's pipeline and set the stage after creation.
  if (!closedWonStageId) {
    try {
      const details = await getDealsByIds([result.dealId], ["pipeline", "dealstage"]);
      const pipeline = details[0]?.properties?.pipeline || "";
      const resolved = pipeline ? await resolveClosedWonStageIdForPipeline(pipeline) : "";
      if (resolved) {
        result.stageUpdate = await updateHubSpotDealStage(result.dealId, resolved);
        result.pipelineId = pipeline;
        result.stageId = resolved;
      } else {
        result.stageUpdate = { updated: false, reason: "closed_won_stage_not_found" };
      }
    } catch (err) {
      console.error(
        "❌ HubSpot stage update error:",
        err?.response?.status,
        err?.response?.data || err
      );
      result.stageUpdate = { updated: false, reason: "error" };
    }
  } else {
    // Stage was set during create; report as such.
    result.stageUpdate = { updated: true, reason: "set_on_create", stageId: closedWonStageId };
  }
...
export async function updateHubSpotDealSlackThread(dealId, threadTs) {
  if (!HUBSPOT_TOKEN) {
    throw new Error("HubSpot token missing.");
  }
  if (!dealId || !threadTs) {
  ...
  const propertyName = HUBSPOT_DEAL_SLACK_THREAD_PROPERTY || "slack_thread_ts";

  await axios.patch(
    `https://api.hubapi.com/crm/v3/objects/deals/${dealId}`,
    { properties: { [propertyName]: String(threadTs) } },
    {
      headers: {
        Authorization: `Bearer ${HUBSPOT_TOKEN}`,
        "Content-Type": "application/json",
  ...
  return { updated: true, dealId, propertyName, value: String(threadTs) };
}
...
async function getCompanyContactAssociations(companyId, limit = 20) {
  if (!HUBSPOT_TOKEN) {
    throw new Error("HubSpot token missing.");
  }
  if (!companyId) return [];
...
export async function createHubSpotDeal({ dealname, pipelineId, stageId }) {
  if (!HUBSPOT_TOKEN) {
    throw new Error("HubSpot token missing.");
  }
  if (!dealname) {
...
export async function getHubSpotPrimaryContactIdForCompany(companyId) {
  if (!companyId) return null;

  let primaryTypeIds = new Set();
  try {
    const labels = await getAssociationLabels("companies", "contacts");
    primaryTypeIds = new Set(
      labels
        .filter((label) => /primary/i.test(label.label || ""))
        .map((label) => label.typeId)
  ...
  const associations = await getCompanyContactAssociations(companyId);
  return pickPrimaryAssociation(associations, primaryTypeIds);
}
