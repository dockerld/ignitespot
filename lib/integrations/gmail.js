...
const GMAIL_IMPERSONATE_USER =
  process.env.GMAIL_IMPERSONATE_USER || "pingautomation@ignitespot.com";
...
export async function sendEmail({ from, to, subject, body }) {
  ...
  // Impersonate pingautomation@ but set From header to the requested address (e.g. qbo@)
  const token = await getAccessToken(GMAIL_IMPERSONATE_USER);
